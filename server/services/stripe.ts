...
const logger = {
  info: (message: string, ...args: any[]) => console.log(`[StripeService] ${message}`, ...args),
  error: (message: string, ...args: any[]) => console.error(`[StripeService] ${message}`, ...args),
  warn: (message: string, ...args: any[]) => console.warn(`[StripeService] ${message}`, ...args)
};
...
async function handlePaymentIntentSucceeded(paymentIntent: Stripe.PaymentIntent) {
  ...
  try {
    ...
    // TODO: Implement user pro access activation when database schema is ready
    if (paymentIntent.metadata?.product === 'pro_calculator') {
      logger.info('Pro Calculator payment successful - would activate user access');
    }
  } catch (error) {
...
async function handlePaymentIntentFailed(paymentIntent: Stripe.PaymentIntent) {
  ...
  try {
    ...
    logger.info('Payment failed - user access not granted');
  } catch (error) {
