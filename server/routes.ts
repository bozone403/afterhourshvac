...
Immediate response required!`;
  ...
  app.post("/api/create-payment-intent", requireAuth, async (req, res) => {
    try {
      const { amount, currency, description, customer_name, project_details } = req.body;
  ...
  app.post("/api/purchase-pro-membership", requireAuth, async (req, res) => {
    try {
      const user = req.user as any;
      const { membershipType } = req.body; // 'monthly', 'yearly', 'lifetime'
