...
function AdminDashboardEnhanced() {
  ...
  const deleteBlogPostMutation = useMutation({
    mutationFn: async (postId: number) => {
      return await apiRequest("DELETE", `/api/admin/blog-posts/${postId}`);
    },
    onSuccess: () => {
      toast({
  ...
  const createTeamMemberMutation = useMutation({
    mutationFn: async (teamData: any) => {
      const res = await apiRequest("POST", "/api/admin/team-members", teamData);
      return await res.json();
    },
    onSuccess: () => {
      toast({
  ...
  const deleteTeamMemberMutation = useMutation({
    mutationFn: async (memberId: number) => {
      return await apiRequest("DELETE", `/api/admin/team-members/${memberId}`);
    },
    onSuccess: () => {
      toast({
  ...
  const handleCreateBlogPost = () => {
    ...
    const slug = newBlogPost.slug || newBlogPost.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
    createBlogPostMutation.mutate({ ...newBlogPost, slug });
  };

  const handleCreateTeamMember = () => {
    if (!newTeamMember.name || !newTeamMember.role || !newTeamMember.description) {
      toast({
        title: "Error",
    ...
    const specialtiesArray = newTeamMember.specialties.split(',').map(s => s.trim()).filter(s => s);
    createTeamMemberMutation.mutate({ ...newTeamMember, specialties: specialtiesArray });
  };
      ...
      <div className="hvac-container space-y-8">
        ...
        <Tabs defaultValue="users" className="space-y-6">
          <TabsList className="hvac-card p-2 bg-gradient-to-r from-blue-50 to-orange-50">
            ...
            <TabsTrigger value="team" className="hvac-text-base px-6 py-3 rounded-xl font-semibold data-[state=active]:bg-gradient-to-r data-[state=active]:from-blue-600 data-[state=active]:to-blue-700 data-[state=active]:text-white">Team Management</TabsTrigger>
          </TabsList>
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        ...
        <TabsContent value="blog" className="space-y-4">
          <div className="hvac-card">
            <div className="flex items-center justify-between mb-6">
              ...
              <Dialog open={showCreateBlogDialog} onOpenChange={setShowCreateBlogDialog}>
                <DialogTrigger asChild>
                  <Button className="hvac-button-primary">
                ...
                <DialogContent className="max-w-4xl">
                  ...
                  <div className="flex justify-end gap-2">
                    <Button variant="outline" onClick={() => setShowCreateBlogDialog(false)}>
                      Cancel
                    </Button>
                    <Button 
                    ...
                    <div key={post.id} className="flex items-center justify-between p-4 border rounded-lg">
                      <div className="flex items-center space-x-4">
                        <div>
                          <div className="flex items-center gap-2">
                            <h4 className="font-semibold">{post.title}</h4>
                            <Badge className={post.published ? 'bg-green-600 text-white' : 'bg-gray-600 text-white'}>
        ...
        <TabsContent value="team" className="space-y-4">
          <div className="hvac-card">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="hvac-heading-md mb-2">Team Member Management</h2>
                <p className="hvac-text-base text-gray-600">Add, edit, and manage team members displayed on the About page</p>
              </div>
                ...
                <DialogContent className="max-w-2xl">
                  ...
                  <div className="grid gap-4 py-4">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <Label htmlFor="team-role">Role/Position</Label>
                        <Input
                          id="team-role"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <Label htmlFor="team-specialties">Specialties (comma-separated)</Label>
                        <Input
                          id="team-specialties"
                          value={newTeamMember.specialties}
                          onChange={(e) => setNewTeamMember({ ...newTeamMember, specialties: e.target.value })}
                          placeholder="Commercial HVAC, System Design, Energy Audits"
                        />
                      </div>
                    </div>
                    <div>
                    ...
                    <div>
                      <Label htmlFor="team-image">Image URL (optional)</Label>
                      <Input
                        id="team-image"
