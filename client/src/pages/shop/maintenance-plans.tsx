...
const MaintenancePlansShop = () => {
  ...
  const handleBuyNow = async (plan: any) => {
    try {
      let price = 0;
      if (plan.oneTimePrice) price = plan.oneTimePrice;
      else if (plan.visitPrice) price = plan.visitPrice;
      else if (plan.annualPrice) price = plan.annualPrice;
      else if (plan.monthlyPrice && !isAnnual) price = plan.monthlyPrice;
      ...
      const response = await fetch('/api/stripe/create-payment-intent', {
        ...
        body: JSON.stringify({
          ...
          currency: 'cad',
          description: `${plan.name} - Maintenance Plan`,
          metadata: {
                  ...
                  <Card key={plan.id} className={`relative bg-white shadow-lg transition-all duration-300 overflow-hidden ${plan.popular ? 'ring-2 ring-blue-500 scale-105' : 'hover:shadow-xl'}`}>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <Button 
                        ...
                        size="lg"
                      >
                        Buy Now - ${plan.annualPrice.toLocaleString()}
                      </Button>
                    </CardContent>
                  </Card>
                ))}
