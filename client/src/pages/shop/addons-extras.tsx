...
const AddOnsExtrasPage = () => {
  ...
  const handleBuyNow = async (item: AddOnItem) => {
    if (item.contactRequired) {
      window.location.href = '/contact';
      return;
    }

    try {
      // Extract numeric price from string like "$650 - $950" or "$400"
      ...
      const response = await fetch('/api/stripe/create-payment-intent', {
        ...
        body: JSON.stringify({
          amount: numericPrice * 100, // Convert to cents
          currency: 'cad',
              ...
              <CardContent className="space-y-4">
                ...
                <div className="pt-2 space-y-2">
                  {item.contactRequired ? (
                    <Button onClick={() => handleBuyNow(item)} className="w-full">
                      <Phone className="h-4 w-4 mr-2" />
                      Contact for Quote
                    </Button>
                  ) : (
                    <>
                      <Button onClick={() => handleBuyNow(item)} className="w-full bg-orange-500 hover:bg-orange-600">
                        Buy Now - {item.price}
                      </Button>
