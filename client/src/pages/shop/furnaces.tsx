...
const FurnacesShop = () => {
  ...
  const handleBuyNow = async (furnace: any) => {
    try {
      const response = await fetch('/api/stripe/create-payment-intent', {
        ...
        body: JSON.stringify({
          amount: furnace.price * 100, // Convert to cents
          currency: 'cad',
          description: `${furnace.name} Installation`,
          metadata: {
            service: 'furnace-purchase',
            model: furnace.name,
            price: furnace.price.toString()
          }
        }),
      });
      ...
      sessionStorage.setItem('pendingPayment', JSON.stringify({
        clientSecret,
        productId: 'furnace',
        totalAmount: furnace.price,
        service: furnace.name
      }));
